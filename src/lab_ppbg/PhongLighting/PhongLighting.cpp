...
void PhongLighting::Init()
{
	...
	{
		Shader *shader = new Shader("PhongLighting");
		shader->AddShader("Source/Laboratoare/PhongLighting/Shaders/PhongLighting.VS.glsl", GL_VERTEX_SHADER);
		shader->AddShader("Source/Laboratoare/PhongLighting/Shaders/PhongLighting.FS.glsl", GL_FRAGMENT_SHADER);
...
void PhongLighting::Update(float deltaTimeSeconds)
{
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(0, 1, 0));
		RenderSimpleMesh(meshes["sphere"], shaders["PhongLighting"], modelMatrix);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["PhongLighting"], modelMatrix);
	}

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["PhongLighting"], modelMatrix, glm::vec3(0, 0.5, 0));
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["PhongLighting"], modelMatrix);
	}
...
void PhongLighting::FrameEnd()
{
	DrawCoordinatSystem();
}

void PhongLighting::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
...
void PhongLighting::OnInputUpdate(float deltaTime, int mods)
{
	float speed = 2;
...
void PhongLighting::OnKeyPress(int key, int mods)
{
	// add key press event
	if (key == GLFW_KEY_1)
	{
...
void PhongLighting::OnKeyRelease(int key, int mods)
{
	// add key release event
}

void PhongLighting::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
	// add mouse move event
}

void PhongLighting::OnMouseBtnPress(int mouseX, int mouseY, int button, int mods)
{
	// add mouse button press event
}

void PhongLighting::OnMouseBtnRelease(int mouseX, int mouseY, int button, int mods)
{
	// add mouse button release event
}

void PhongLighting::OnMouseScroll(int mouseX, int mouseY, int offsetX, int offsetY)
{
}

void PhongLighting::OnWindowResize(int width, int height)
{
}
